...
FORCEINLINE FMatrix3x3::FMatrix3x3(const FMatrix4x4 & Matrix) {
	FMatrix3x3(Matrix.Row(0), Matrix.Row(1), Matrix.Row(2));
}

FORCEINLINE FMatrix3x3::FMatrix3x3(const FVector3 & Row0, const FVector3 & Row1, const FVector3 & Row2) {
...
inline FVector3 FMatrix3x3::Row(const int & i) const {
	switch (i) {
		case 0: return FVector3(m0[0], m0[1], m0[2]);
		case 1: return FVector3(m1[0], m1[1], m1[2]);
		case 2: return FVector3(m2[0], m2[1], m2[2]);
	}

	return FVector3();
}

inline FVector3 FMatrix3x3::Column(const int & i) const {
	switch (i) {
		case 0: return FVector3(m0[0], m1[0], m2[0]);
		case 1: return FVector3(m0[1], m1[1], m2[1]);
		case 2: return FVector3(m0[2], m1[2], m2[2]);
	}

	return FVector3();
}

inline FVector3 & FMatrix3x3::operator[](unsigned int i) {
	switch (i) {
		case 0:  return m0;
		case 1:  return m1;
		case 2:  return m2;
...
inline FVector3 const & FMatrix3x3::operator[](unsigned int i) const {
	switch (i) {
		case 0:  return m0;
		case 1:  return m1;
		case 2:  return m2;
...
FORCEINLINE FVector3 FMatrix3x3::operator*(const FVector3& Vector) const {
	FVector3 result(
		Row(0).Dot(Vector),
		Row(1).Dot(Vector),
		Row(2).Dot(Vector)
