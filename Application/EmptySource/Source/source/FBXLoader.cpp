...
bool FBXLoader::LoadScene(FbxScene * pScene, FileStream * File) {
	...
	const bool lImportStatus = lImporter->Initialize(WStringToString(File->GetPath()).c_str(), -1, gSdkManager->GetIOSettings());
	...
	if (!lImportStatus) {
		...
		Debug::Log(Debug::LogError, L"Import failed, error returned : %s", CharToWChar(error.Buffer()));
