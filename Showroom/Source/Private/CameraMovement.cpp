...
CCameraMovement::CCameraMovement(ESource::GGameObject & GameObject)
	: CComponent(L"CameraMovement", GameObject), GameStateComponent(NULL) {
}
...
void CCameraMovement::OnInputEvent(ESource::InputEvent & InEvent) {
	...
	if (GameStateComponent == NULL || GameStateComponent->GameState != CGameState::EGameState::Started) return;
...
void CCameraMovement::OnUpdate(const ESource::Timestamp & DeltaTime) {
	if (GameStateComponent != NULL && GameStateComponent->GameState == CGameState::EGameState::Started) {
		...
		if (InputIndex == 1 && ESource::Input::IsMouseDown(ESource::EMouseButton::Mouse2)) {
			...
			CameraRotation = Quaternion::FromEulerAngles(EulerAngles + 
				Vector3(ESource::Input::GetMouseY() - LastCursorPosition.Y, -ESource::Input::GetMouseX() - -LastCursorPosition.X)
			);
		}
		...
		CameraRotation = Quaternion::FromEulerAngles(
			EulerAngles + Vector3(AxisY * ViewSpeed, -AxisX * ViewSpeed, 0.F) * MathConstants::RadToDegree 
			* ESource::Time::GetDeltaTime<ESource::Time::Second>()
		);
		...
		LastCursorPosition = { ESource::Input::GetMouseX(), ESource::Input::GetMouseY() };
		CameraRotation = GetGameObject().LocalTransform.Rotation;
		LastCameraRotation = CameraRotation;
	}
}
