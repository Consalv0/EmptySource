...
void CFollowTarget::OnUpdate(const ESource::Timestamp & DeltaTime) {
	if (Target != NULL) {
		...
		if (ModuleMovement > 0.0F) {
			TargetPosition.X = Math::Abs(TargetPosition.X - ThisTransform.Position.X) >= ModuleMovement ?
				TargetPosition.X - std::fmodf(TargetPosition.X, ModuleMovement) : ThisTransform.Position.X;
			TargetPosition.Y = Math::Abs(TargetPosition.Y - ThisTransform.Position.Y) >= ModuleMovement ?
				TargetPosition.Y - std::fmodf(TargetPosition.Y, ModuleMovement) : ThisTransform.Position.Y;
			TargetPosition.Z = Math::Abs(TargetPosition.Z - ThisTransform.Position.Z) >= ModuleMovement ?
		...
		ThisTransform.Position = Math::Mix(ThisTransform.Position, TargetPosition, DeltaSpeed);
	}
}
