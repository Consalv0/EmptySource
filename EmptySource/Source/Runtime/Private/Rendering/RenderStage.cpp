...
namespace ESource {
	...
	void RenderStage::End() {
		...
		if (RenderShader && RenderShader->IsValid()) {
			...
			RenderShader->GetProgram()->SetTexture("_DepthTexture", Application::GetInstance()->GetRenderPipeline().GetGBufferTexture(GB_Depth)->GetTexture(), 3);
